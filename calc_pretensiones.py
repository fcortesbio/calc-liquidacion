...
# --- 1. Carga y Procesamiento de Datos de los Desprendibles de Nómina ---
...
for month_period_obj, group in df_paystubs.groupby('year_month_period'):
    ...
    monthly_salary = monthly_base_salary + monthly_extras  # Salario mensual = Salario base + Extras
    ...
    monthly_summary_list.append({
        ...
        "Aporte Pensión (12%)": round(monthly_aporte_pension)  # Redondea el valor final
    })
...
# --- 3. Cálculo de la Liquidación (Prestaciones Sociales) ---

# Define las fechas clave relacionadas con el contrato
fecha_inicio_contrato = datetime(2023, 4, 17)
fecha_fin_contrato = datetime(2024, 2, 17)  # Fecha de terminación del contrato
...
fecha_inicio_cesantias_2024 = datetime(2024, 1, 1); fecha_fin_cesantias_2024 = datetime(2024, 1, 1); fecha_fin_cesantias_2024 = fecha_fin_contrato # Corrección: El periodo va hasta la fecha de fin de contrato
...
liquidacion_items = [
    {"Concepto": "Prima 1er Semestre 2023 (Abr 17 - Jun 30)", "Valor": prima_1_2023, "Estado": "Pagada"},
    {"Concepto": "Prima 2do Semestre 2023 (Jul 01 - Dic 31)", "Valor": prima_2_2023, "Estado": "Pagada"},
    {"Concepto": "Prima Proporcional 2024 (Ene 01 - Feb 17)", "Valor": prima_2024, "Estado": "No Pagada"},
    {"Concepto": "Cesantías 2023 (Abr 17 - Dic 31)", "Valor": cesantias_2023, "Estado": "No Pagada"},
    ...
    {"Concepto": "Vacaciones (Abr 17, 2023 - Feb 17, 2024)", "Valor": vacaciones, "Estado": "No Pagada"}
]
df_liquidacion = pd.DataFrame(liquidacion_items)
df_liquidacion["Valor"] = df_liquidacion["Valor"].round().astype(int)  # Redondea los valores a enteros
...
# --- 4. Cálculo de Indemnizaciones por Mora ---
fecha_actual = datetime.now()  # Obtiene la fecha y hora actual para el cálculo en tiempo real
# Para pruebas con una fecha específica, puedes descomentar la siguiente línea y ajustarla:
# fecha_actual = datetime(2025, 5, 8)

# Salario diario base para el cálculo de las indemnizaciones.
# Se toma el último salario mensual nominal y se divide por 30.
ultimo_salario_mensual_ref = 2_100_000
salario_diario_indemnizacion_general = ultimo_salario_mensual_ref / 30

# 4.1. Indemnización por mora en el pago de la liquidación (Artículo 65 del CST)
# El periodo de mora inicia 15 días hábiles después de la terminación del contrato (17 de febrero de 2024).
# Sin embargo, el código calcula 15 días calendario para simplificar.
...
# 4.2. Sanción por no consignación oportuna de las Cesantías 2023 (Artículo 99 de la Ley 50 de 1990)
...
if salario_mensual_vigente_dic2023 == 0:  # En caso de no encontrar datos de diciembre, usa el salario de referencia
...
# Reporte de Indemnizaciones y Sanciones por Mora
...
# --- 5. Monto Total de las Pretensiones ---
